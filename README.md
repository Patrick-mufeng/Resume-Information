...
基于多模型 AI 视觉 API 的简历信息批量提取工具，支持 **6 大 AI 提供商** + **多 Key 轮询** + **PDF/Word 自动转图片** + **自定义提取字段**。
...
- **多 Key 轮询**：添加多个 API Key，自动交替使用突破速率限制
- **Key 可用性检测**：添加 Key 自动验证，绿灯有效 / 红灯无效 / 灰灯无法验证
- **PDF/Word 自动转图片**：上传或扫描文件夹时自动转换，跳过空白页
- **文件夹批量处理**：输入文件夹路径一键扫描 → 转换 → 识别
- **自定义提取字段**：自由增删提取字段，AI 严格按字段列表输出 JSON
- **实时进度监控**：WebSocket 推送日志流 + 进度条 + 统计卡片
- **暂停/恢复**：处理中途可随时暂停
- **CSV 导出**：动态字段表头，兼容 Excel
- **数据持久化**：SQLite 存储所有历史记录，支持重试失败项
...
3. 点击 **扫描并处理**，系统自动：
   - 扫描文件夹内所有 PDF/Word/JPG/PNG
   - PDF/Word 转为图片（已有图片则跳过）
   - 删除空白页
   - 创建任务 + 自动启动 AI 识别

**方式二：手动上传**
1. 点击 **新建任务**
2. 拖拽文件到上传区（支持 JPG/PNG/PDF/DOCX）
3. 点击 **开始处理**

### 第三步：自定义提取字段（可选）

进入 **提取设置** 页面，自由增删字段。修改后 AI 提取和 CSV 导出自动适配。
...
- **后端**：Python FastAPI + SQLAlchemy + SQLite + WebSocket
